...
\begin{defn}
\textbf{Back-door criterion} A set of variables $Z$ satisfy the back-door criterion relative to an ordered pair of variables ($X_i$,$X_j$) in a DAG $G$ if:
\begin{enumerate}[noitemsep]
...
\begin{defn}
\textbf{Front-door} A set of variables $Z$ satisfy the front-door criterion relative to an ordered pair of variables $(X, Y)$ if:
\begin{enumerate}[noitemsep]
...
\begin{thm}
\textbf{Front-door adjustment} If $Z$ satisfies the front-door criterion relative to $(X, Y)$ and if $P(x, z) > 0$, then the causal effect of $X$ on $Y$ is identifiable and is given by
\begin{equation}
...
\begin{thm}
\textbf{Rule 1} Insertion/deletion of observations: \label{thm:do-calc-ins-del-obs}
\begin{equation}
...
\begin{thm}
\textbf{Rule 2} Action/observation exchange: \label{thm:do-calc-act-obs-ex}
\begin{equation}
...
\begin{thm}
\textbf{Rule 3} Insertion/deletion of actions:  \label{thm:do-calc-ins-del-act}
\begin{equation}
...
\begin{defn}
\textbf{Simpson's paradox} The phenomenon whereby an event $C$ increases the probability of $E$ in a given population $p$ and, at the same time, decreases the probability of $E$ in every sub-population of $p$. In other words, if $F$ and $\neg F$ are two complementary properties describing two sub-populations, then it is possible to encounter the equalities:
\begin{align}
P(E|C) &> P(E|\neg C) \\
P(E|C,F) &< P(E| \neg C, F) \nonumber \\
P(E|C, \neg F) &< P(E| \neg C, \neg F). \nonumber
