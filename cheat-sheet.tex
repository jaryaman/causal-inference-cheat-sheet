...
\begin{thm}[Wright's rules for path analysis]
Assuming standardized variables in a linear DAG, the total expected correlation between two variables is equal to the sum of the contribution of all pathways through which the two variables are connected. The strength of each pathway is the product of path-coefficients along the pathway. To trace a path:
\begin{enumerate}[noitemsep]
\item You can never pass out of one arrowhead and then into another arrowhead (no colliders)
\item You can pass though each variable only once in a given chain of paths
\item No more than one bidirectional arrow (denoting confounded variables) can be included in each path-chain
\end{enumerate}
\end{thm}
...
\begin{thm}[Criterion for stable no-confounding]
